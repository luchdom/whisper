...
`flush` finalizes current speech and waits for its original final events plus every accepted translation update, then emits `flushed`; capture can continue in the same session. `stop` already performs a flush, drains final inference and accepted translation work, then emits `session_stopped`. `shutdown` stops an active session, closes both workers and their models, emits `shutdown`, and exits. A client should send `stop` or `flush`, not `stop` followed by `flush`.
...
Partial and final transcript events have the same segment shape. The canonical original final is emitted before local translation inference:
...
{"type":"final_segment","session_id":"session","segment":{"id":"session:system:000001","revision":2,"start_ms":1000,"end_ms":2300,"track":"system","text":"final text","partial":false,"final":true,"language":"en","speaker_id":"speaker-01","translated_text":null,"translated_language":null}}
{"type":"segment_translation","session_id":"session","segment_id":"session:system:000001","segment_revision":2,"translated_text":"texto final","translated_language":"pt-BR"}
```
...
`text` is always the canonical original transcript. Partials never carry a translation. An eligible original final is emitted immediately with nullable translation fields, then one bounded translation worker may emit `segment_translation` for that exact `session_id`, `segment_id`, and `segment_revision`. Clients apply an update only to an existing finalized exact revision; an orphan, stale, future, or duplicate update is ignored. This enrichment is not another transcript revision and must not advance original-text Assist or debrief context. `translated_language` is exactly `pt-BR`. Translation is attempted only for explicit/English-only input or auto-detected English with a finite confidence of at least `0.80`. Embedded final translation fields remain valid for protocol compatibility.
...
{"type":"warning","source":"transcription","code":"translation_backpressure","message":"Brazilian Portuguese translation is falling behind; original transcription will continue and some translations may be unavailable","recoverable":true}
...
`source` is `protocol`, `capture`, or `transcription`. A transcription error can include `segment_id`. `inference_backpressure` is emitted once and contains no PCM, transcript, participant, path, or model details. `translation_backpressure` is also emitted at most once per session; it drops only overflowed optional translation work and never blocks audio ingestion or original final delivery.
...
Finalized eligible English is tokenized with SentencePiece 0.2.2 using the separate `>>pob<<` target token. Input is limited to 4,000 characters and 512 total source pieces, output to 512 pieces, and inference to a batch of one. Original text remains canonical and is emitted before translation begins. A dedicated single-consumer queue keeps the mutable CTranslate2 runtime off the ASR worker and bounds pending phrases; its asynchronous updates retain original final order. An empty finalized ASR segment is treated as silence/non-speech and skipped without disabling translation for later speech. Preparation or genuine per-segment translation errors emit at most one sanitized `translation_unavailable` warning per session; queue saturation emits one `translation_backpressure` warning and skips only overflowed translations. Neither failure sets `final_inference_failed` or removes the original final. `flush` and `stop` drain accepted translations before completion so the parent can reconcile bilingual autosave deterministically. macOS translation remains disabled until deterministic converted outputs and runtime behavior are verified there. Source identity and licenses are recorded in [../THIRD_PARTY_NOTICES.md](../THIRD_PARTY_NOTICES.md).
