...
The current source release is **v0.9.2**. It retains the v0.9 capture/runtime, provider, DPAPI, and native-acceptance hardening, narrowly recognizes only macOS's validated fixed `/var` and `/tmp` compatibility aliases during fail-closed model provisioning, and verifies both Windows `resources` and macOS `Contents/Resources` package layouts; arbitrary model-directory links and reparse points remain rejected. The Windows standalone bundle still includes its Python runtime and native dependencies, passes a content-free component probe and JSONL shutdown smoke without global Python, and ships a CycloneDX SBOM plus notices. It is not yet an Authenticode-signed, clean-machine-qualified release; Developer ID signing, notarization, macOS hardware acceptance, the named capture-app matrix, and real 60-minute Windows/macOS desktop soaks remain external gates. The earlier Windows sidecar-only hour passes sustained ASR/translation/diarization load but does not cover Electron capture, UI reconciliation, autosave, or Portuguese semantic quality. Hosted Assist remains explicit and source-implemented but has not been exercised against the live OpenAI API. Meeting detection, overlap-aware diarization, cross-session voice identity, automatic/background assistance, structured fact/citation presentation, provider-generated summaries, and a fully local generative model remain future work. See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md), [docs/DISTRIBUTION.md](docs/DISTRIBUTION.md), [docs/PLATFORM-COMPATIBILITY.md](docs/PLATFORM-COMPATIBILITY.md), and [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md).
