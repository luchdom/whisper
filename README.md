...
The current source release is **v0.6.0**. The repository has a one-command source bootstrap, but does not yet ship a standalone bundled Python/model runtime. The Windows sidecar has passed its strict 60-minute release-scope ASR/translation/diarization-load gate. The actual desktop capture/autosave path and all macOS capture/performance behavior remain runtime-unverified, and Portuguese semantic quality was not scored by the payload-focused soak. Local translation remains Windows x64-only until its deterministic artifacts and behavior are verified on macOS. The workspace, overlay, shortcut, persistence, hardening, and display-recovery contracts have deterministic coverage. An isolated Windows source-app runtime acceptance also passed at 1440, 1120, 880, and 760-pixel widths, including the separate focused permission dialog, Accessible/Private mode transitions, content protection/taskbar policy, settings persistence, and non-persisted click-through recovery; operating-system shortcut conflicts were reported as unavailable without disabling the rest. Actual macOS content protection, always-on-top behavior, global shortcuts, menu-bar/login-item behavior, capture, and Assist remain In Review until exercised on hardware. The explicit hosted Assist flow, including meeting profiles and OS-encrypted private context, is source-implemented but has not been exercised against the live OpenAI API. Signing, notarization, meeting detection, overlap-aware diarization, cross-session voice identity, automatic/background assistance, structured fact/citation output, summaries, and a fully local assistance model remain future work. See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) for the replaceable-engine boundary and staged roadmap, and [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) for model attribution and immutable source identities.
