...
The current source release is **v0.8.0**. The repository has both a one-command source bootstrap and a platform-specific standalone-runtime build path. The current Windows bundle includes the Python runtime and native dependencies, passes a content-free component probe and JSONL shutdown smoke without using global Python, and ships a CycloneDX SBOM plus notices. It is not yet an Authenticode-signed, clean-machine-qualified release; the macOS ARM workflow, Developer ID signing, notarization, and hardware acceptance also remain pending. The Windows sidecar has passed its strict 60-minute release-scope ASR/translation/diarization-load gate. The actual desktop capture/autosave path and all macOS capture/performance behavior remain runtime-unverified, and Portuguese semantic quality was not scored by the payload-focused soak. Local translation remains Windows x64-only until its deterministic artifacts and behavior are verified on macOS. The workspace, overlay, shortcut, persistence, hardening, display-recovery, and local-debrief implementation have deterministic coverage and local Windows validation. The isolated v0.6.0 Windows source-app acceptance still covers the 1440/1120/880/760-pixel layouts, focused permission dialog, Accessible/Private transitions, content-protection/taskbar policy, settings restart persistence, and non-persisted click-through recovery. The debrief's synthetic benchmark establishes bounded runtime and honest partial-state handling, not real-meeting quality. A real 60-minute debrief still needs review for accuracy, attribution, false positives, and exported-file privacy, and actual macOS debrief behavior remains In Review until exercised on hardware. The explicit hosted Assist flow, including meeting profiles and OS-encrypted private context, is source-implemented but has not been exercised against the live OpenAI API; OpenAI is not used by the local debrief. Meeting detection, overlap-aware diarization, cross-session voice identity, automatic/background assistance, structured provider fact/citation output, provider-generated summaries, and a fully local generative assistance model remain future work. See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) for the replaceable-engine boundary, [docs/DISTRIBUTION.md](docs/DISTRIBUTION.md) for release gates, and [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) for model/runtime attribution and immutable source identities.
