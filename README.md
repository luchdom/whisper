...
A one-sentence smoke translation took **0.109 seconds** on the validated Windows machine. More importantly, a pre-gate wall-clock backend run exercised effectively the first hour of a public multi-speaker English panel at the app's 200 ms packet granularity using `small.en`, online anonymous speaker clustering, and local pt-BR translation. All **288 non-empty finalized segments** carried a non-empty payload labeled pt-BR; two empty non-speech finals were correctly skipped; no warning, inference error, or backpressure event occurred; and the session stopped cleanly. This is strong translation-payload and sustained-pipeline evidence, not independent linguistic quality or completion of the current strict release gate. The test found and fixed a bug where one empty final previously disabled translation for the rest of the meeting.

That evidence validates the Windows sidecar's long-session ASR/translation path, not the whole desktop capture path. The anonymous clustering reached its 16-cluster safety cap on a panel with fewer expected participants, so the run is load/stability evidence for diarization—not an accuracy pass; threshold and reconciliation work remain. The backend feed also bypasses Windows loopback selection, Electron capture/resampling/IPC, renderer reconciliation, and autosave. A one-hour YouTube-at-1x run through the actual app is still required for that separate end-to-end gate. Source, revision, file-hash, and license details are recorded in [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md).
...
The pre-gate hour run with `small.en` + speaker clustering + pt-BR translation measured the Python sidecar process tree separately: **1,360.6 MiB peak resident working set** and **4,789.5 MiB peak private commit**. Median private commit was **4,342.5 MiB** in the first stable ten-minute window and **4,289.4 MiB** in the last ten minutes, a **53.2 MiB decrease** rather than a monotonic-growth signal. Final-event latency was 4.009 seconds p50 and 6.828 seconds p95 overall; first-ten-minute p95 was 6.731 seconds and last-ten-minute p95 was 7.044 seconds. These figures include ASR, translation, and diarization but exclude the Electron process tree.
...
The repository has a one-command source bootstrap, but does not yet ship a standalone bundled Python/model runtime. The Windows sidecar has strong pre-gate hour evidence, but a full hour under the current strict release evaluator is still pending after an interrupted rerun. The actual desktop capture/autosave path and all macOS capture/performance behavior also remain runtime-unverified. Local translation remains Windows x64-only until its deterministic artifacts and behavior are verified on macOS. Signing, notarization, tray mode, meeting detection, overlap-aware diarization, cross-session voice identity, summaries, and an in-meeting copilot remain future work. See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) for the replaceable-engine boundary and staged roadmap, and [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) for model attribution and immutable source identities.
