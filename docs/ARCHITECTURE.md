...
The automated soak compresses 60 virtual minutes into a deterministic queue/state regression and reports its limitation in its output. It verifies bounded queue accounting and drainage, not real-time ASR throughput, translation throughput, meeting-capture durability, or speaker accuracy. A separate pre-gate wall-clock backend run fed effectively the first hour of a public multi-speaker recording through the production JSONL sidecar at the application's 200 ms packet cadence with real `small.en` inference, online diarization, and local English-to-pt-BR translation. It is strong backend-pipeline and sustained-throughput evidence, but it predates the current strict evaluator and bypasses desktop system-audio capture, Electron IPC, renderer reconciliation, and autosave. A current release-scope sidecar rerun plus a real 60-minute Windows desktop meeting and a real 60-minute macOS meeting with working devices remain required.

A one-sentence English-to-pt-BR smoke completed in 0.109 seconds on the validated Windows machine. The pre-gate wall-clock run then emitted a non-empty pt-BR-labeled translation payload for all 288 non-empty final segments, skipped two empty finals, emitted no warnings or errors, and stopped cleanly. Resident memory peaked at 1,360.6 MiB, while private-memory medians decreased by 53.2 MiB from the first stable window to the last; final-segment latency was 4.009 seconds at p50 and 6.828 seconds at p95. This does not yet complete the strict Windows backend translation-pipeline gate or independently assess Portuguese meaning. Windows desktop capture/autosave and macOS runtime gates remain outstanding, and the 16-cluster diarization safety cap reached during the source does not establish speaker-label accuracy.
